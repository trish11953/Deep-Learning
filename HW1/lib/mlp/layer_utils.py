...
class fc(object):
    ...
    def backward(self, dprev):
        ...
        #############################################################################
        weight_grad = self.meta.T.dot(dprev)
        bias_grad = np.ones((1, dprev.shape[0])).dot(dprev).reshape(self.output_dim, )
        self.grads[self.w_name] = weight_grad
        self.grads[self.b_name] = bias_grad
...
class gelu(object):
    ...
    def backward(self, dprev):
        ...
        # Store the output gradients in the variable dfeat provided above.          #
        #############################################################################
        dGELU = 0.5 * np.tanh(0.0356774 * feat ** 3 + 0.797885 * feat) + 0.5 + \
                (0.0535161 * feat ** 3 + 0.398942 * feat) * (1 / np.cosh(0.0356774 * feat ** 3 + 0.797885 * feat)) ** 2
        dfeat = dprev * dGELU
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
...
class dropout(object):
    ...
    def forward(self, feat, is_training=True, seed=None):
        ...
        sol = None
        if is_training:
            seed_t = self.rng.uniform(0.0, 1.0, feat.shape) <= self.keep_prob
            kept = seed_t .astype(int)
            sol = feat * seed_t.astype(int) * (1.0 / self.keep_prob)
        else:
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        #############################################################################
        one_hot = np.zeros(feat.shape)
        one_hot[np.arange(len(label)), label] = 1
        label = one_hot
        loss = - np.sum(np.multiply(one_hot, np.log(logit))) / feat.shape[0]
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.logit = logit
...
def softmax(feat):
    ...
    #############################################################################
    x_max = np.max(feat, axis=1, keepdims=True)
    x_exp = np.exp(feat - x_max)
    # Computing the sum of exponentiated values along each row
    row_sums = np.sum(x_exp, axis=1, keepdims=True)
    # Computing the softmax values by dividing the exponentiated values by row sums
    scores = x_exp / row_sums
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    return scores
